...
class InternVL3Embedder(nn.Module):
    def __init__(
        ...
        if self.use_da3:
            ...
            class DPTLinearFilM(nn.Module):
                def __init__(self, dim_in=1536, dim_out=896):
                    super().__init__()
    ...
    def apply_film(self, features, gama, beta):
        
        return features * gama + beta       
    ...
    def get_fused_image_text_embedding_from_tensor_images(
        ...
        if self.use_da3:
            ...
            func = lambda x: self.dpt_embedder(x,image_mask)
            dpt_embeds = checkpoint(func,image_tensors, use_reentrant=False)
        else:
            fused_embeds = vit_embeds  
        ...
        fused_hidden = fused_hidden[:, 0, :] if return_cls_only else fused_hidden

        if self.use_da3:
            gama, beta = self.dpt_linear_film(dpt_embeds)
            fused = self.apply_film(fused_hidden, gama, beta)
